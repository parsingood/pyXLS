...
subject = "Easter occupancy report"
body = "Easter occupancy report"
...
paxrooms=[('Rooms','e.QUANTITY',', e.QUANTITY'),
          ('Pax','(e.ADULTS + e.CHILDREN)',', e.ADULTS, e.CHILDREN')]

hotelagents = [
    ('R.SEASON5 RESORT, R.TOT_ROOMS "All"','e.resort,',
...
for hotel_list in HOTELS:
    ...
    for hotelagent in hotelagents:
        for paxroom in paxrooms:
            ...
            ## - Table
            worksheet.merge_range(cr, 0, cr, days+2, ' '+paxroom[0]+' for Easter 2023', merge_bold  )
            cr+=2
            conn = cx_Oracle.connect(user=r'opera', password='opera', dsn=dsn_tns) # if needed, place an 'r' before any parameter in order to address special characters such as '\'. For example, if your user name contains '\', you'll need to place 'r' before the user name: user=r'User Name'
            cursor = conn.cursor()
            cursor.execute(s)
    ...
    worksheet.write(cr, 0, 'Дата: ' + TODAY_STR)
    cr=cr+1
    worksheet.write(cr, 0, 'Изготвили: ')
