...
for hotel_list in HOTELS:
    ...
    if yesterday:
        if (today - yesterday).days == 1:
            title = f"Движение през {yesterday} на нощувки за периода {for_period} в {hotel_list[1]} по туроператори и хотели"
            ...
            title = f"Движение в интервала {yesterday.strftime('%d.%m')}-{(today - datetime.timedelta(days=1)).strftime('%d.%m')} на нощувки за периода {for_period} в {hotel_list[1]} по туроператори и хотели"
        ...
        title = f"Нощувки до {today.strftime('%d.%m.%Y %a')} за периода {for_period} в {hotel_list[1]} по туроператори и хотели"
        body  = f"Нощувки постъпили до {today.strftime('%d.%m.%Y %a')} за периода {for_period} по туроператори и хотели"
    ...
    ## - Table 1
    worksheet.merge_range(cr, 0, cr, 11, title, merge_bold  )
