...
receiver_email = ["maya.lazarova@albena.bg","ivanm@albena.bg","viktoriya.valkova@albena.bg"]
...
sql='''
Select TourOperator, RESORT ''' + column + '''
from(SELECT TourOperator, RESORT,  the_date
    ...
    and TO_DATE(\''''+ datetime.datetime(year_this, month_day_till[0], month_day_till[1]).strftime("%Y%m%d") + '''\','YYYYMMDD') 
     ...
     and SR.COMPANY not in ('STF','CONTRACTS','Block for future overbookings','MNG-KLIMENT KOLEV')
...
for hotel_list in HOTELS:
    ...
    f"Движение през {yesterday} на нощувки за месец {for_month}/{year_this} в {hotel_list[1]}"
    , merge_bold  )
...
subject = f"UpDown for Month {for_month}/{year_this} Overnights at {yesterday_day}"
body = f"Промяна в нощувките за месец {for_month}/{year_this} през деня {yesterday_day}"
