...
receiver_email = ["mtodorova@albena.bg","velina.gyumova@albena.bg","ivanm@albena.bg", "marinela.tsaneva@albena.bg"]
receiver_email = ["ivanm@albena.bg"]
subject = f"Нощувки за Великден към {yesterday_day}"
body = "Приложена е справката за нощувките за Великден."
...
hotel_info = {
    ...
    "KLK": ("KALIAKRA", 272),
    "DTC": ("KALIAKRA MARE", 148),
    "ORL": ("SANDY BEACH", 163),
    "MAL": ("MALIBU", 157),
    "DOR": ("MARITIM PARADISE", 238),
    "DRU": ("MARITIM AMELIA", 141),
    "OAS": ("OASIS", 162),
    "FLG": ("FLAMINGO GRAND", 181),  #263
    "FLA": ("FLAMINGO", 186),
    "OR1": ("ORCHIDEA", 210),
    "MAG": ("MAGNOLIA", 340),
...
workbook = xlsxwriter.Workbook('C:/test/HotelNightStays.xlsx')
...
worksheet.set_column(0, 0, 5)  # Ширина за кода на хотела
worksheet.set_column(1, 1, 20)  # Увеличена ширина за името на хотела
...
row = 0
worksheet.write_row(row, 0, ["Резервирани стаи за Великден по хотели и туроператори"], header_format)
row += 1
row += 1
...
with open('C:/test/HotelNightStays.xlsx', "rb") as attachment:
    part = MIMEBase("application", "octet-stream")
    part.set_payload(attachment.read())
    encoders.encode_base64(part)
    part.add_header("Content-Disposition", f"attachment; filename= HotelNightStays.xlsx")
    message.attach(part)
