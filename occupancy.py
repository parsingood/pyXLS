...
parser = argparse.ArgumentParser(
    description="Справка за заетост на хотели: гости по дни, по тур-агенти и др.",
    epilog="Пример: py occupancy.py --date1 2025-04-24 --date2 2025-05-07 --pax adch",
    add_help=True
)
# Четем аргументите
parser = argparse.ArgumentParser(description="Генериране на справка за заетост и гости по хотели.")
parser.add_argument('--date1', type=str, required=True, help="Начална дата във формат YYYY-MM-DD")
parser.add_argument('--days', type=int, help="Брой дни напред от началната дата")
parser.add_argument('--date2', type=str, help="Крайна дата във формат YYYY-MM-DD")
parser.add_argument('--pax', type=str, choices=["adch"], default=None,
                    help="adch = отделно възрастни и деца, ако липсва => общо възрастни+деца")
parser.add_argument('--hotels', type=str, help="Списък хотели --hotels=GER-SLA-MRA-BOR, ако hotels липсва ще даде всички хотели в Албена")
parser.add_argument('--email', type=str, help="Имели на които да се праща --email=marinela.tsaneva@albena.bg;mtodorova@albena.bg;velina.gyumova@albena.bg;ivanm@albena.bg;maya.lazarova@albena.bg")
...
date1 = datetime.datetime.strptime(args.date1, "%Y-%m-%d").date()

if args.date2:
    date2 = datetime.datetime.strptime(args.date2, "%Y-%m-%d").date()
    days = (date2 - date1).days  # Ако има date2, изчисляваме days
elif args.days is not None:
    days = args.days
    date2 = date1 + datetime.timedelta(days=days)  # Ако има days, изчисляваме date2
else:
    print("Грешка: трябва да подадеш или --days, или --date2.")
    sys.exit(1)

# Показваме резултатите
print(f"Начална дата: {date1}")
print(f"Крайна дата: {date2}")
print(f"Брой дни: {days}")


subject = f"Заетост {date1.strftime('%d.%m')}-{date2.strftime('%d.%m.%Y')} { 'възр.-деца' if args.pax == 'adch' else '' } по хотели и тур-агенти"
body = f"Заетост {date1.strftime('%d.%m')}-{date2.strftime('%d.%m.%Y')} { 'възрастни и деца по отделно' if args.pax == 'adch' else '' } по хотели и тур-агенти в нощувки и стаи, според постъпилите резервации към момента, без незаетите блокирани по гаранции и out-of-order стаи"
...
to_email ="marinela.tsaneva@albena.bg" #"ivanm@albena.bg"
receiver_email =["marinela.tsaneva@albena.bg","ivanm@albena.bg"] #,"ivanm.albena@gmail.com"
...
sender_email = "ivanm.albena@gmail.com"
sender_password = "npdkxrfpkmiwqmng" #"mzlqlnrrpjtasxra"
mailserver = "smtp.gmail.com"
...
'FLA', 'OR1', 'OR2', 'MAG', 'SUP', 'RAL', 'VIT', 'KOM', 'ALT', 'KPS', 'PAN', 'VMG','GOR','MUR'
''','Albena'),
#(''' 'MUR' ''','White Lagoon'),
...
if args.hotels:
    hotel_list=args.hotels.split("-")
    HOTELSTR="', '".join(hotel_list)
    HOTELSTR=f"'{HOTELSTR}'"
    HOTELS = [(HOTELSTR,'Albena')]

paxrooms=[
('СТАИ','e.QUANTITY AD, 0 CH',', e.QUANTITY'),
('НОЩУВКИ',' e.ADULTS AD , e.CHILDREN CH ',', e.ADULTS, e.CHILDREN')
]
...
if args.pax == 'adch':
    cols_with_sum = list(range(1, 2*days + 2))  # AD и CH колони за всеки ден
else:
    cols_with_sum = list(range(1, days + 2))    # само една колона на ден
...
columns_this = ''
for d in range(days):
    date_formatted = (date1 + datetime.timedelta(d)).strftime("%Y%m%d")
    date_label = (date1 + datetime.timedelta(d)).strftime("%d_%m")
    
    if args.pax == 'adch':
        columns_this += f'''
        ,SUM(case when the_date=trunc(TO_DATE('{date_formatted}','YYYYMMDD'), 'DDD') then AD else 0 end) "{date_label} AD"
        ,SUM(case when the_date=trunc(TO_DATE('{date_formatted}','YYYYMMDD'), 'DDD') then CH else 0 end) "{date_label} CH"
        '''
    elif args.pax == 'room' or args.pax == 'rooms':
        columns_this += f'''
        ,SUM(case when the_date=trunc(TO_DATE('{date_formatted}','YYYYMMDD'), 'DDD') then (AD+CH) else 0 end) "{date_label}"
        '''
    else:
        columns_this += f'''
        ,SUM(case when the_date=trunc(TO_DATE('{date_formatted}','YYYYMMDD'), 'DDD') then (AD+CH) else 0 end) "{date_label}"
...
if args.file == "test" or args.file == "exit":
    exit()
