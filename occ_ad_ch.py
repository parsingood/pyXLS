...
subject = f"Заетост {date1.strftime('%d.%m')}-{date2.strftime('%d.%m.%Y')} по хотели и тур-агенти"
body = f"Заетост {date1.strftime('%d.%m')}-{date2.strftime('%d.%m.%Y')} по хотели и тур-агенти в нощувки и стаи, според постъпилите резервации към момента, без незаетите блокирани по гаранции и out-of-order стаи"
