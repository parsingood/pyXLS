...
FROM (
    ...
    FROM (
        ...
        WHERE e.ISSUE_DATE BETWEEN TO_DATE('{yesterday.strftime('%Y%m%d')}', 'YYYYMMDD')
...
) OCC JOIN OPERA.RESORT R ON R.RESORT = OCC.RESORT
...
for hotel_list in HOTELS:
    ...
    worksheet.merge_range(cr, 0, cr, 11, 
