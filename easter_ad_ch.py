...
subject = "Великден по хотели и тур-агенти"
body = "Великден по хотели и тур-агенти в нощувки и стаи, според постъпилите резервации към момента, без незаетите блокирани по гаранции и out-of-order стаи"
...
paxrooms=[('НОЩУВКИ',' e.ADULTS AD , e.CHILDREN CH ',', e.ADULTS, e.CHILDREN')]
...
date1 = datetime.date(2025,4,15)  # datetime.date.today()
#if date1 < datetime.date(2024,4,30):
#    date1 = datetime.date(2024,4,30)
days = 8 #31
date2 = date1 + datetime.timedelta(days)
cols_ignore = [] 
#cols_with_sum = list(range(4, 4+days+1))
#cols_with_sum.extend(range(4+days+5, 4+days + 5 + days))
cols_with_sum = list(range(1, 2*days+2))
...
for d in timedelta_list:
    ...
    , 'DDD') then AD else 0 end) "''' + (date1 + datetime.timedelta(d)).strftime("%d_%m") + ''' AD" '''

    columns_this += ''',SUM(case when the_date=trunc(
    TO_DATE(\'''' + (date1 + datetime.timedelta(d)).strftime("%Y%m%d") + '''\','YYYYMMDD')
    , 'DDD') then CH else 0 end) "''' + (date1 + datetime.timedelta(d)).strftime("%d_%m") + ''' CH" '''
