...
receiver_email = ["mtodorova@albena.bg","maya.lazarova@albena.bg","ivanm@albena.bg"]
...
, SUM(OVNTS) OVNTS
 ...
 AND R.TRUNC_END_DATE > TO_DATE('28122022','DDMMYYYY')
 AND R.TRUNC_BEGIN_DATE<= TO_DATE('04012023','DDMMYYYY')
 AND R.RESORT = '@hotel'
...
, SUM(AD)+SUM(CH) PAX
 ...
 WHERE (R.RESV_STATUS <> 'CANCELLED') AND (R.RESV_STATUS <> 'NO SHOW') 
   AND NVL(RT.PSEUDO_YN ,'N')='N'
   AND (EN.ADULTS + EN.CHILDREN > 0) 
 AND R.TRUNC_END_DATE > TO_DATE('28122021','DDMMYYYY')
 AND R.TRUNC_BEGIN_DATE<= TO_DATE('04012022','DDMMYYYY')
