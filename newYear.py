...
subject = "Reservations daily report - New Year"
body = "Reservations daily report - New Year"
...
, SUM(OVNTS) OVNTS
 ...
 AND R.TRUNC_END_DATE > TO_DATE('28122024','DDMMYYYY')
 AND R.TRUNC_BEGIN_DATE<= TO_DATE('04012025','DDMMYYYY')
 AND R.RESORT = '@hotel'
...
CASE WHEN DOR_RL > 0 THEN '+' || TO_CHAR(DOR_RL) ELSE ' ' END DOR_RL, SUM(DOR_RL) OVER (ORDER BY INSERT_DATE) DOR_RL_T,
CASE WHEN DOR_RT > 0 THEN '+' || TO_CHAR(DOR_RT) ELSE ' ' END DOR_RT, SUM(DOR_RT) OVER (ORDER BY INSERT_DATE) DOR_RT_T, 
CASE WHEN DOR_PL > 0 THEN '+' || TO_CHAR(DOR_PL) ELSE ' ' END DOR_PL, SUM(DOR_PL) OVER (ORDER BY INSERT_DATE) DOR_PL_T, 
CASE WHEN DOR_PT > 0 THEN '+' || TO_CHAR(DOR_PT) ELSE ' ' END DOR_PT, SUM(DOR_PT) OVER (ORDER BY INSERT_DATE) DOR_PT_T 
,
...
FROM (
select INSERT_DATE,
SUM(case when HOTEL = 'DOR' THEN RL ELSE 0 END) DOR_RL,
SUM(case when HOTEL = 'DOR' THEN PL ELSE 0 END) DOR_PL,
SUM(case when HOTEL = 'DOR' THEN RT ELSE 0 END) DOR_RT,
SUM(case when HOTEL = 'DOR' THEN PT ELSE 0 END) DOR_PT,
...
, SUM(AD)+SUM(CH) PAX
 ...
 WHERE (R.RESV_STATUS <> 'CANCELLED') AND (R.RESV_STATUS <> 'NO SHOW') 
   AND NVL(RT.PSEUDO_YN ,'N')='N'
   AND (EN.ADULTS + EN.CHILDREN > 0) 
 AND R.TRUNC_END_DATE > TO_DATE('28122023','DDMMYYYY')
 AND R.TRUNC_BEGIN_DATE<= TO_DATE('04012024','DDMMYYYY')
 AND R.RESORT IN ('DOR', 'FLG')
AND SO.COMPANY NOT IN (
  'KT-SPECIAL GUESTS',
  'CONTRACTORS','STF-ERROR1','XX'
  )
...
workbook = xlsxwriter.Workbook('C:/test/NewYear-'+dt_string+'.xlsx')
...
## - Table 1
worksheet.merge_range(cr, 0, cr, 6, 'резервациите за нова година '
...
for sumCol in cols_with_sum:
	worksheet.write_formula(cr, sumCol, '='+ xl_rowcol_to_cell(r_tot1, sumCol) +'+'+  xl_rowcol_to_cell(r_tot2, sumCol) +'', bold, total1[sumCol] + total2[sumCol]) 
#	worksheet.write_number(cr, sumCol, total1[sumCol] + total2[sumCol] , bold)
	worksheet.write(cr, sumCol, total1[sumCol] + total2[sumCol] , bold)
cr=cr+2
...
cols = {
'DOR_RL':{'w':w1,'h':'last','f':border_plus},
'DOR_RL_T':{'w':w2,'h':'rooms','f':border},
'DOR_RT':{'w':w1,'h':'this','f':border_plus},
'DOR_RT_T':{'w':w2,'h':'rooms','f':border},

'DOR_PL':{'w':w1,'h':'last','f':border_plus},
'DOR_PL_T':{'w':w2,'h':'pax','f':border},
'DOR_PT':{'w':w1,'h':'this','f':border_plus},
'DOR_PT_T':{'w':w2,'h':'pax','f':border},

'FLG_RL':{'w':w1,'h':'last','f':border_plus},
'FLG_RL_T':{'w':w2,'h':'rooms','f':border},
'FLG_RT':{'w':w1,'h':'this','f':border_plus},
'FLG_RT_T':{'w':w2,'h':'rooms','f':border},

'FLG_PL':{'w':w1,'h':'last','f':border_plus},
'FLG_PL_T':{'w':w2,'h':'pax','f':border},
'FLG_PT':{'w':w1,'h':'this','f':border_plus},
...
files = [f for f in all_files if re.match(r'^NewYear.+\.xlsx', f)]
