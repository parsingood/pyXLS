...
sql='''
select bill_no,   ';' || gen || ';' || hotel || ';' || voucher || ';;' 
       || arr || ';' || dep || ';' || names || ';' ||  pax || ';'  info,
sum(gross) amount,
startdate || '_' || hotel || '_' || to_char(bill_no) filename 
from (   
     SELECT F.bill_no,
    ...
    join  financial_transactions ft on f.resort = ft.resort and f.bill_no = ft.bill_no  
     ...
     join name n on rn.NAME_ID = n.NAME_ID    
      left JOIN RESORT H ON H.RESORT = SUBSTR(en.RATE_CODE,6,3)
...
GROUP BY bill_no,gen,hotel,voucher,arr,dep,names, pax, startdate
...
bill_no = ''
total = 0
#f = open(path + "zerro.txt", "w")
f = open(path + "zerro.txt", "w", encoding="utf-8")
f.write("Empty file \n end"  )
...
#       filename = str(bill_no)
    ...
    f.write( str(row[0])+row[1]+ str(row[2]).replace('.',',')+';'+ str(row[2]).replace('.',',')+';EUR^M\n')
    total=total + row[2]
