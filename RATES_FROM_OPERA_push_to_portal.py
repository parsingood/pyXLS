...
def pax_variants(
    ...
    gchild_age = 2, # Минимален години на плащащите (порасналите) деца
    ...
    if adult_age == gchild_age :
        ...
        for adults, children in product(
            ...
            if min_pax <= adults + children <= max_pax:
                ...
                # [12,12,2,   2,0,1,0],  задължителни нули втората позиция : gchildren = 0

    elif gchild_age == child_age :
        # Генериране на всички комбинации на възрастни и деца
        for adults, gchildren in product(
            range(min_ad, max_ad + 1), 
            range(min_ch, max_ch + 1)
        ):
            if min_pax <= adults + gchildren <= max_pax:
                combinations.append ([adult_age, gchild_age, child_age,  adults, gchildren, 0, 0] )
                # [12,2,2,   2,1,0,0],  задължителни нули третат позиция : children = 0
    else:
...
for HOTEL in HOTELS:
    ...
    ORDER BY RESORT, RATE_CODE, ROOMTYPES, BEGIN_DATE
        ...
        for row in rows:
            ...
            # Цена за допълнителен възрастен
            ex=row[9]
            # Цените за децата
            ...
            for roomtype in rt:
                if (hotel,roomtype) in rms:
                    ...
                    for paxpick_filter in paxpick_filters:
                        ...
                        gc=paxpick_filter.get("gch") # count of grown children 6-11 или 2-11
                        c=paxpick_filter.get("ch") # count of children 2-5 или нищо (2-2) 

                        # максималния включен в наема брой възрастни и деца
                        rent_max = rents.get((rate, hotel, roomtype), rents.get((hotel, roomtype)))
                        #print(f"rate, hotel, roomtype: {rate},{hotel},{roomtype}")
                        #print(f"rent_max:{rent_max}")

                        if rent_max:
                            # наема се взема от втората позиция 
                            am = ad2 + a * 1 #  по 1 евро на възрастен надценка
                            try:
                                #rent_max = {'max_ad': 2, 'max_ch': 2}
                                a_max = rent_max.get('max_ad',2)
                                c_max = rent_max.get('max_ch',1)
                                print(f"a_max:{a_max}, c_max:{c_max} - am:{am}, a:{a}, c:{c}")
                                if a > a_max:
                                    am += (a-a_max) * ex 
